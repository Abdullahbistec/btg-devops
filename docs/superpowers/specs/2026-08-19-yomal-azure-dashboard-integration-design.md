...
| `SavingsCard` | Sub-project B's `monthly_cost`/`monthly_saving` | Azure (`idle` today; wider once Sub-project A's `cost-analysis` lands, pending the overlap resolution); Hetzner once a pricing lookup is added to its waste checks (not in this design's scope — flagged, not built); PP shows an explicit "not available" state, not zero. **Currency blocker — see Risks table:** these columns mix EUR (Hetzner) and USD (Azure) with no currency dimension; do not build a cross-provider sum here until that's resolved. |
...
| **`monthly_cost`/`monthly_saving` mix currencies with no currency dimension.** Confirmed during Phase 1's implementation: Hetzner's volume waste estimate is EUR (list-price, `cmd/hetzner_volumes.go`'s `est_monthly_waste_eur`), while Azure `idle`'s figures are USD actual billed spend (`cmd/usage.go`'s `TotalCost`/`TotalSaving`) — both land in the same `monthly_cost` column with no currency recorded, even though `report.Currency` already exists in Go and is currently discarded. `SUM(monthly_cost)` across providers is meaningless as-is. | **Blocking for `SavingsCard` (Phase 9) and any other aggregate over these columns** — do not ship a cross-provider sum until either a `currency` column is added alongside, or `monthly_cost`/`monthly_saving` are scoped per-provider so no aggregate silently mixes them. Not fixed in Phase 1 (schema/plumbing only) — deliberately parked as a Phase 9 precondition rather than expanding Phase 1's scope. |
